...
struct
{
    ...
    // simple boolean for winning the game
    bool won;
} g;
...
bool change_num(int num);
bool check_num(int num, bool status);
bool check_win(void);
void solve(void);
void shutdown(void);
bool startup(void);
...
main(int argc, char *argv[])
{
    ...
    do
    {
        ...
        // move cursor if getch() returns a directional key
        move_cursor(ch);

        // change number if getch() returns a number
        // if the number is changed, check for win
        if (change_num(ch) && check_win()) {
            show_banner("Congratulations! You won!");
            g.won = true;
            draw_numbers();
        } 
...
draw_numbers(void)
{
    ...
    for (int i = 0; i < 9; i++)
    {
        ...
        {
            // determine char
            char c = (g.altboard[i][j] == 0) ? '.' : g.altboard[i][j] + '0';

            if (has_colors()) {
                if (!g.won && g.altboard[i][j] != g.board[i][j])
...
show_cursor(void)
{
    ...
    g.absy = g.top + g.y + 1 + g.y/3;
    g.absx = g.left + 2 + 2*(g.x + g.x/3);
...
/*
 * Moves the cursor to position depending on the pressed key
 */
...
move_cursor(int key)
{
    ...
    //char message[50];
    //sprintf(message, "absy = %d, absx = %d", g.absy, g.absx);
    //show_banner(message);
}

/*
 * Changes the selected slot with a dot (.) to the pressed number
 */

bool
change_num(int num)
{
    // if the game is won, then don't change anything
    if (!g.won) {
        // change number if its between 1 and 9 and if it's a dot on the original board
        if (num > '0' && num <= '9' && check_num(num, true)) {
            if (g.board[g.y][g.x] == 0) {
                g.altboard[g.y][g.x] = num - ('a' - '1');
                draw_numbers();
                return true;
            }
            else
                show_banner("You can't change this number!");
        }
        else if (num == KEY_BACKSPACE || num == KEY_DC || num == '0') {
            if (g.board[g.y][g.x] == 0) {
                g.altboard[g.y][g.x] = 0;
                draw_numbers();
            }
        }
    }
...
/*
 * Checks if the put number isn't wrong
 */

bool
check_num(int num, bool status)
{
    int num2 = num;
    if (num > 9)
        num2 -='0';
    ...
    for (int x = 0; x < 9; x++) {
        if (num2 == g.altboard[g.y][x]) {
            if (status) {
                char message[50];
                sprintf(message, "The current number %d can't be put here!", num2);
                show_banner(message);
            }
            return false;
        }
        if (num2 == g.altboard[x][g.x]) {
            if (status) {
                char message[50];
                sprintf(message, "The current number %d can't be put here!", num2);
                show_banner(message);
    ...
    // square check
    // minimum and maximum values to the for loop
    ...
    //char message[50];
    //sprintf(message, "Debugging: num = %d, g.y = %d, g.x = %d", num, g.y, g.x);
    //show_banner(message);
    return true;
}
...
check_win(void)
{
    // checks one by one from the whole grid, in case something's wrong, returns false
    for (int i = 0; i < 9; i++) {
        for (int j = 0; j < 9; j++) {
            if (g.altboard[i][j] != 0) {
                // horizontal and vertical checks
                for (int x = 0; x < 9; x++) {
                    if (j != x && g.altboard[i][j] == g.altboard[i][x]) {
                        return false;
                    }
                    if (j != x && g.altboard[j][i] == g.altboard[x][i]) {
                        return false;
                    }
                }
                // square check
                ...
                for (int a = mina; a < maxa; a++) {
                    for (int b = minb; b < maxb; b++) {
                        if (i != a && j != b && g.altboard[i][j] == g.altboard[a][b]) {
                            return false;
                        }
...
solve(void)
{
    int possible, num2, bkpy = g.y, bkpx = g.x, fy, fx;
    bool status = true;
    char message[80], numbers[20], number[5];
    for (int i = 0; i < 9; i++) {
        for (int j = 0; j < 9; j++) {
            if (g.board[i][j] == 0) {
                possible = 0, g.y = i, g.x = j; 
                sprintf(numbers, "");
                if (g.altboard[i][j] != 0 && check_num(g.altboard[i][j], false)) {
                    continue;
                }
                ...
                //sprintf(message, "Possible numbers (%d): %s, Looking at: %d, %d, g.y = %d, g.x = %d",
                //        possible, numbers, i, j, g.y, g.x);
                //show_banner(message);
                g.y = bkpy, g.x = bkpx;
                if (possible == 1) {
                    g.altboard[i][j] = num2;
                ...
                if (possible == 2) {
                    sprintf(message, "Possible numbers to be put here: %s", numbers);
                    status = false;
                    fy = i, fx = j;
                } else if (possible == 3 && status) {
                    sprintf(message, "Possible numbers to be put here: %s", numbers);
                    fy = i, fx = j;
                }
            }
